...
res_to_remove = sorted(closed_parm.residues, key=res_max_z)[-n_wat_to_remove:]
...
for parm in (closed_parm, open_parm):
    ...
    # Start from 2
    for i, res in enumerate(parm.residues[:300]):
        res.chain = 'A'
        res.number = i + 2

    # Separate chain for ions
    for i, res in enumerate(parm.residues[300:306]):
        res.chain = 'S'
        res.number = i + 1

    # Split waters into chains of max 9999 residues to avoid problems
    # when loading pdb
    for i, res in enumerate(parm.residues[306:]):
